...
#response_cdc = request(api) |> req_url_query(`$limit`=10000000000) |> req_perform()

#cases_raw = resp_body_json(response_cdc, simplifyVector = TRUE)
...
```{r, message = FALSE}
deaths |> filter(state %in% population$state_name) |> group_by(state) |> summarize(`Total Deaths` = sum(covid_19_deaths, na.rm = TRUE)) |> top_n(10) |> arrange(desc(`Total Deaths`)) |> ggplot() + aes(x = reorder(state, -`Total Deaths`), y = `Total Deaths`) + geom_col() + labs(x = "State", y = "Total Deaths", title = "Deaths by Covid-19 (Top 10 States)") + theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 8))
```